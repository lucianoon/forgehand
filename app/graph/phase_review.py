...
Só o replan depende de NodeDependencies (advisor); ele recebe as dependências
explicitamente. Os demais nós são funções de módulo sobre o estado.
"""
...
async def consult_advisor(
    deps: NodeDependencies, task: AgentTask, state: WorkflowState
...
def _replanned_task(
    task: AgentTask, changes: list[str] | None, advice: AdvisingOutcome | None
) -> AgentTask:
    """Devolve a tarefa a PENDING com as correções do judge e do advisor."""
    ...
    return task.model_copy(update=task_update)


async def replan_rejected_tasks(
...
def _unsupported_strategy(state: WorkflowState) -> bool:
    return (
...
def gate_reason(state: WorkflowState) -> str:
    """Motivo objetivo da pausa, na ordem de precedência do gate."""
    ci_failed = (
        state.delivery_result is not None and state.delivery_result.ci_failed
    )
    if _unsupported_strategy(state):
        return "unsupported_build_strategy"
    if (
        ...
        return f"factory_delivery_{state.delivery_result.ci_state}"
    if state.budget_blocked_reason and state.budget_blocked_reason.startswith(
        "llm_call_failed"
    ):
        return "llm_call_failed"
    if state.budget_exhausted:
        return "budget_exhausted"
    if state.iterations_exhausted and state.tasks_needing_replan:
        return (
            "ci_failed_iterations_exhausted"
            if ci_failed
            else "iterations_exhausted"
        )
    if state.escalated_tasks:
        return "tasks_escalated"
    if ci_failed:
        return "ci_failed"
    return "dependency_deadlock"


def gate_payload(state: WorkflowState, reason: str) -> dict[str, Any]:
    """O que o humano vê no interrupt: motivo, evidência e opções."""
    options = (
        ["retry", "abort"]
        if _unsupported_strategy(state) or state.work_order is not None
        else [
            "accept_partial",
            "retry",
            "abort",
...
async def human_gate(state: WorkflowState) -> dict[str, Any]:
    decision = interrupt(gate_payload(state, gate_reason(state)))
...
# --------------------------------------------------------------------------
# Retry autorizado, síntese e abort
# --------------------------------------------------------------------------


def _retry_task_budget(task_budget: TaskBudget, *, blocked: bool) -> TaskBudget:
    """Dobra o teto da tarefa escalada que bateu no limite (ou foi bloqueada)."""
    ...
    if blocked or task_tokens >= task_budget.max_tokens:
        budget_updates["max_tokens"] = 2 * max(task_budget.max_tokens, task_tokens)
    if blocked or task_cost >= task_budget.max_cost_usd:
        budget_updates["max_cost_usd"] = 2 * max(task_budget.max_cost_usd, task_cost)
    return task_budget.model_copy(update=budget_updates)
    ...
    retryable_escalations: list[AgentTask] = [
        task.model_copy(
            update={
                ...
                "budget": _retry_task_budget(
                    task.budget, blocked=state.budget_blocked_reason is not None
                ),
                "updated_at": datetime.now(timezone.utc),
            }
        )
...
def build_review_nodes(deps: NodeDependencies) -> dict[str, Any]:
    """Amarra os nós da fase aos nomes do workflow; só o replan usa deps."""

    async def replan(state: WorkflowState) -> dict[str, Any]:
        return await replan_rejected_tasks(deps, state)
